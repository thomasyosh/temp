...
class Main {
        ...
        while (serviceCode != 0) {
            ...
            switch (serviceCode) {
                ...
                case 2:
                    ...
                    reportMessage += "(2 gift)";
                    totalActionCount++;
                    break;
                case 3:
                    q.enqueue(0);
                    reportMessage += "(1 check bal)";
                    totalActionCount++;
                    break;
                case 4:
                    for (int i = 2; i >= 0; i--) {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < minutes + 1; i++) {
            ...
            if (i != 1)
                reportMessage = reportMessage.replaceFirst("]", "");

            if (!q.isEmpty()) {
                if ((int) q.front() != 0) {
                    int res = new Scanner(reportMessage).useDelimiter("\\D+").nextInt();
                    reportMessage = reportMessage.replaceFirst(String.valueOf(res), String.valueOf(res - 1));
                }
